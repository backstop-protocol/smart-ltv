...
contract ReallocatesDAiToUSDT is Script {
    ...
    returns (MarketAllocation[] memory allocations, RiskData[] memory riskDatas, Signature[] memory signatures)
  {
    ...
    allocations[0] = MarketAllocation({marketParams: marketParamSDAI, assets: 15000000000});
    console.log("allocations 0 supply: %s", allocations[0].assets);
    allocations[1] = MarketAllocation({marketParams: marketParamUSDT, assets: 15000000000});
    ...
    generateRiskDataAndSignature(marketParamUSDT, 1, riskDatas, signatures);
    console.log("riskdata 1 collateral: %s", riskDatas[1].collateralAsset);
    console.logBytes32(signatures[1].r);
    generateRiskDataAndSignature(marketParamIdle, 2, riskDatas, signatures);
